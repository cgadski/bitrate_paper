...
Let $X_1, \ldots, X_n$ be independent Rademacher random variables (i.e., $\Pp(X_i = 1) = \Pp(X_i = -1) = 1/2$), and let $S_n = X_1 + \cdots + X_n$. Then for any $t > 0$,
$$
\Pp(S_n \ge t) \le \exp\left(-\frac{t^2}{2n}\right).
$$
\end{proposition}

\begin{proof}
For a single Rademacher variable $X_i$, we have $\E e^{\lambda X_i} = \frac{1}{2}(e^{\lambda} + e^{-\lambda}) = \cosh(\lambda) \le e^{\lambda^2/2}$ for all $\lambda \in \mathbb{R}$. By independence, $\E e^{\lambda S_n} = \prod_{i=1}^n \E e^{\lambda X_i} \le e^{n\lambda^2/2}$. Applying the Chernoff inequality,
$$
\Pp(S_n \ge t) \le e^{-\lambda t} \E e^{\lambda S_n} \le e^{-\lambda t + n\lambda^2/2}.
$$
Minimizing over $\lambda$ by setting $\lambda = t/n$ yields the result.
\end{proof}
...
\{ v \in S^{d - 1} : v_1 \ge t \}
$$
under the uniform probability measure on $S^{d - 1}.$ Using the fact that we can construct a uniform distribution on the unit sphere by normalizing a Gaussian random vector, it is possible to establish that
$$
b(d, t) \le (1 + o(1)) \exp\left(- \frac {t^2 d} 2 d \right)
$$
