...
\begin{corollary}
	Let $d \ge 8 k^2 (2 \ln N + \ln p^{-1}),$ and let $F \in \R^{d \times N}$ be a dictionary of random codewords in the conditions of \Cref{prop:many-ortho}. Then with probability at least $(1 - p),$ every $k$-element subset $Y \subseteq [N]$ is recovered from its superposition code $F Y$ by threshold decoding. \label{prop:quadratic-condition}
\end{corollary}
