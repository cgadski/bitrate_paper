...
One natural approach to designing the dictionary $F$ is to assign each codeword $f_i$ to a random unit-norm vector. This is motivated by the fact that, so long as $d = \Omega(\ln d),$ we can expect random vectors to be nearly orthogonal. More precisely, we have results like the following.
...
\begin{proposition}
	...
	\begin{equation}
	\forall i \neq j, \; \lvert \langle F_i, F_j \rangle \rvert < \epsilon \label{eq:interference}
	\end{equation}
	with probability at least $(1 - p).$
\end{proposition}

Given a dictionary $F$ of unit-norm vectors satisfying (\ref{eq:interference}) with $\epsilon = 1/2k,$ the vector $y$ can be decoded from the superposition code $F y$ in one obvious way: specifically, by applying the thresholding rule
$$
\sigma(t) = \begin{cases}
	1 : t \ge 1/2 \\
	0 : \text{otherwise}
\end{cases}
$$
element-wise to the vector $F^T F y.$ \Cref{prop:many-ortho} thus guarantees that, when encoding sets of a fixed size $k,$ we only need $O(\ln N)$ dimensions for this simple thresholding procedure to be reliable.

\subsection{One-Step Methods}

We will a \textbf{one-step method} any 
\begin{equation}
