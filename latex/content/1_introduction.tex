...
\item We investigate a simple extension of top-$k$ which we call the \textbf{generalized matching pursuit} (GMP). Numerical experiments show that, for the same value of $(N, d),$ GMP can decode superposition codes carrying more than two times as much information while requiring only a small constant factor times more computation than top-$k$.
\end{enumerate}
...
\item \Cref{sec:information} briefly realls the idea of a Shannon limit and explains two different heuristic predictions for the number of dimensions that may be required for a sparse superposition code to be decodable.
\item \Cref{sec:main} gives our main theoretical results on the performance of threshold decoding and our experimental comparison with matching-$k.$
\item Finally, \Cref{sec:dictionaries} considers the question of whether
\end{itemize}
