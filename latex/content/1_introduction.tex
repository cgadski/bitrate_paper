...
If each neuron in a given neural network coded for a ``meaningful'' feature of its input, we could hope to reverse-engineer this network's overall behavior on a neuron-by-neuron basis. However, both language models and vision models have been found to learn neurons that correlate simultaneously with apparently unrelated features. This phenomenon is known as polysemanticity. (See for example \citet{nguyen_multifaceted_2016}, \citet{zhang_sample_2023} and \citet{olah_zoom_2020}.)
...
In many classical situations, this question is answered by well-known ideas from information theory. Given a ``channel'' with certain characteristics---for example, a band-limited telephone connection or a binary storage device with a certain failure rate---the amount of information we can encode is asymptotically characterized by a ``channel capacity'' measured in bits per unit of channel usage. The goal of this paper is to study the carrying capacity of sparse superposition codes in a regime that is meaningful to sparse autoencoders. Our analysis focuses on a toy example, described in more detail in \Cref{sec:codes}.

Our main findings are the following.

\begin{enumerate}
\item In the asymptotic regime where $\ln k \le \eta \ln N$ for $\eta < 1,$ we prove that very simple ``one-step methods'' used by sparse autoencoders can reliably decode superposition codes when $d$ is a constant factor $C(\eta)$ greater than the Shannon limit. Moreover, we give concrete lower bounds on the effective capacity $C(\eta)$ which agree well with empirical simulation.
\item On the other hand, we show that $C(\eta)$ is surprisingly large for values of $(N, k)$ typical in applications of sparse autoencoders. In particular, we show empirically that there is a large gap between the
\end{enumerate}
