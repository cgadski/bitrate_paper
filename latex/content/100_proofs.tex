...
\begin{proposition*}
    ...
    $$
    for random spherical dictionaries there exists some constant $C < 1$ such that
    $$
    b(d, k, N; \text{top-$k$}) < C.
    $$
\end{proposition*}
...
\begin{lemma}
    Let $F \in \R^{N \times d}$ be a spherical dictionary, and let $Y = F_1 + \dots F_k$ be the sum of the first $k$ codewords. Then
    $$
    \Pp\bigl(\lVert Y \rVert \ge \sqrt{k} \; \land \; \langle F_1, Y \rangle \le 1\bigr) \ge C
    $$
    for some constant $C > 0$ independent of the parameters $(d, k, N).$
\end{lemma}
...
For each $i > k$, the pair $(F_i, Y / \lVert Y \rVert)$ is a pair of independent, uniform draws from the unit sphere, so combining \Cref{prop:ball} with our assumption $\lVert Y \rVert \ge \sqrt k$ gives
