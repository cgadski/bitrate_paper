...
\end{align*}
The maximum possible signal-to-noise ratio is $\lVert f \rVert_\Sigma^2,$ which is achieved by $\lambda.$ Given an improper uniform prior on $S,$ \eqref{eq:gaussian-posterior} shows the posterior on $S$ conditional on $X$ is a Gaussian with mean $\lambda(X)$ and precision $\lVert f \rVert^2_\Sigma.$
