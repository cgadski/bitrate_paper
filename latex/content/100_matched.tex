...
\end{align*}
Up to a scalar, $\lambda$ can be characterized as the linear function that maximizes this quantity. Specifically, $\lambda$ achieves a signal-to-noise ratio of $\lVert f \rVert_\Sigma^2.$ Given an improper uniform prior on $S,$ \Cref{eq:gaussian-posterior} shows the posterior on $S$ conditional on $X$ is a Gaussian with mean $\lambda(X)$ and precision $\lVert f \rVert^2_\Sigma.$
